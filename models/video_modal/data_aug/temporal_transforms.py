...
class TemporalCenterCrop(object):
    ...
    def __call__(self, frame_indices):
        ...
        center_index = frame_indices.shape[0] // 2
        begin_index = max(0, center_index - (self.size // 2))
        end_index = min(begin_index + self.size, frame_indices.shape[0])
        # print(center_index, begin_index, end_index)
...
class TemporalEvenSample(object):
    def __init__(self, size):
        self.size = size
        self.loop = LoopPadding(size)
    ...
    def __call__(self, frame_indices):
        ...
        frame_indices = range(frame_indices.shape[0])
        n_frames = len(frame_indices)
        stride = max(1, math.ceil(((n_frames - 1) / self.size)))
        out = []
...
class TemporalEvenCrop(object):
    ...
    def __call__(self, frame_indices):
        frames = frame_indices
        frame_indices = range(len(frame_indices))
        n_frames = len(frame_indices)
        ...
        for begin_index in frame_indices[::stride]:
            ...
            if len(sample) < self.size:
                out.append([frames[idx] for idx in self.loop(sample)])

            else:
                out.append([frames[idx] for idx in sample])
